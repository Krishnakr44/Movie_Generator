 ...
 * Generates the next chapter using story state memory. AI must not violate state.
 */
...
import { GenerateChapterSchema } from "@/lib/api/validate";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { storyId, direction, userControls, chapterGoal } = parsed.data;
    ...
    if (story.structure === "acts" && story.actCount) {
      const actSize = Math.ceil(20 / story.actCount); // placeholder: e.g. 20 ch in 3 acts
      nextAct = Math.min(
        story.actCount,
    ...
    const { systemPrompt, userPrompt } = buildUnifiedPrompts({
      title: story.title,
    ...
    console.error("[story/generate-chapter]", e);
